...
void insertSortAdaptado(struct Fabricante *vetor, int n, int f, int h)
{
    int i, j, k, cmp;
    ...
    for (i = f + h; i < n; i += h)
    {
        ...
        while (j >= f && k == f)
        {
            cmp = compararFabricantes(&valor, &vetor[j]);
...
void shellSort(struct Fabricante *vetor, int n)
{
    int p, j, h, np = n / 2;
    ...
    for (p = np; p > 0; p--)
    {
        ...
        for (j = 0; j < h; j++)
        {
            insertSortAdaptado(vetor, n, j, h);
        }
    }
}
